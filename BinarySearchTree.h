...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node *copy_nodes_impl(Node *node) {
    ...
    freshNode->datum = node->datum;
    freshNode->left = copy_nodes_impl(node->left);
    freshNode->right = copy_nodes_impl(node->right);
    return freshNode;
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) {
    if (empty_impl(node)) {
      Node *freshNode = new Node;
      freshNode->datum = item;
      freshNode->left = nullptr;
      freshNode->right = nullptr;
      return freshNode;
    }
  ...
  // NOTE: This function is used in the implementation of the ++ operator for
  //       the iterator code that is provided for you.
  // HINT: At each step, compare 'val' with the current node (using the
