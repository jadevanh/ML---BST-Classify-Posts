...
public:
  ...
  size_t size() const {
    return static_cast<size_t>(tree.size());
  }
  ...
  Iterator find(const Key_type& k) const {
    std::pair<Key_type, Value_type> twins;
    twins.first = k;
    twins.second = Value_type();
    return tree.find(twins);
  }
  ...
  Value_type& operator[](const Key_type& k) {
    std::pair<Key_type, Value_type> freshtwins;
    freshtwins.first = k;
    freshtwins.second = Value_type();
    if (tree.find(freshtwins) == tree.end()) {
      tree.insert(freshtwins);
    }
    return (*(tree.find(freshtwins))).second;
  }
  ...
  std::pair<Iterator, bool> insert(const Pair_type &val) {
    std::pair<Iterator, bool> freshtwins;
    if (tree.find(val) == end()) {
      tree.insert(val);
      freshtwins.second = true;
    } else {
      freshtwins.second = false;
    }
    freshtwins.first = tree.find(val);
    return freshtwins;
  }
